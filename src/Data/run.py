...
def run_accounting_only (ticker=ticker_data, ratio=ratio,
         optional_ratio_fields=optional_ratio_fields,
         monthly_price_path=monthly_price_path,
         accounting_path=accounting_path,
         applicable_ticker_path=applicable_ticker_path,
         )->tuple[pd.DataFrame, dict[str, set[str]]]:
    """Re-fetch only accounting data from SEC EDGAR and recompute
    applicable_ticker, reusing the existing monthly_price.csv on disk
    instead of re-pulling prices from yfinance."""
    multi_tick_price: pd.DataFrame = pd.read_csv(
        monthly_price_path, index_col=['ticker', 'date'], parse_dates=['date'])
    ...
    if not accounting_listframe:
        raise ValueError('No ticker succeeded; nothing to save.')

    multi_tick_accounting: pd.DataFrame = pd.concat(
        accounting_listframe,axis=0).sort_index(level='date')
    multi_tick_price = multi_tick_price.sort_index(level='date')
    ...
    return multi_tick_accounting, ticker_overtime
