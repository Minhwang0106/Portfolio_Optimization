...
def generator (acc_path: Path = accounting_path,
    ...
    """Build the five firm characteristics the proposed model trains on.

    Four are DuPont-style decompositions of the quarterly accounting panel, plus
    one market ratio:
    ...
    * `pb` -- price to book, `close * shares / book_value`.

    Accounting data is quarterly and price data monthly, so `pb` exists only on
    the quarter-end dates both panels share; the other four are quarterly
    throughout. Division by zero and by negative book values is mapped to NaN
    rather than left as +/-inf, so downstream estimators see a missing value
    instead of a number that silently destroys a fit.

    Args:
        acc_path (Path): Accounting panel CSV, indexed (ticker, date), with
            columns 'net_income', 'revenue', 'total_assets', 'book_value',
            'minority_interest', 'preferred_stock'. Defaults to
    ...
    Returns:
        pd.DataFrame: Indexed by (ticker, date) and lexsorted, with columns
            ['ros', 'ato', 'ate', 'g', 'pb'].
    ...
    Note:
        ...
        1e6), `ato` (3.7e7) and `pb` (6.5e8) values all trace back to one of
        these rows rather than to anything economic. Winsorize, or screen on a
        row's scale against the ticker's own median, before fitting.

    Example:
        >>> generator().loc['A'].head(3)
                         ros       ato       ate         g        pb
        date
        2008-09-30  0.155976  0.211360  2.738179       NaN  3.001251
        2009-06-30 -0.017975  0.160809  2.640820       NaN  3.274468
        2009-09-30  0.021422  0.153311  3.047238  0.099002  2.750260
    """
    ...
    #Return on Sales
    col_val['ros'] = acc_df['net_income']/acc_df['revenue']

    #Assets turnover
    col_val['ato'] = acc_df['revenue']/acc_df['total_assets']
    ...
    col_val['ate'] = acc_df['total_assets']/book_values
...
def take_training_data (df:pd.DataFrame, date: str|pd.Timestamp,
    ...
    Args:
        df (pd.DataFrame): Characteristic panel from `generator`, indexed
            (ticker, date). Re-sorted here if it is not already lexsorted; the
    ...
    Returns:
        tuple[pd.DataFrame, pd.DataFrame]: `(df_train, df_future)`, each a slice
            of `df` over the requested tickers and its window.
    ...
    Example:
        >>> tr, fu = take_training_data(generator(), '2020-06-30', ['A', 'AAPL'])
